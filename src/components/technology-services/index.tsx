import React, {useEffect} from "react"
import {TimelineLite} from "gsap"

let reactc = null;
let nodec = null;
let sqlc = null;
let nextc = null;
let mongoc = null;
let gatsbyc = null;
let restc = null;
let graphc = null;
let tl = new TimelineLite();
export default () => {


    useEffect(() => {
        tl.fromTo([
            reactc, sqlc, mongoc, restc
        ], 2, {
            y: 0,
            repeat: -1,
            ease: "none",
            yoyo: true
        }, {
            y: 10,
            repeat: -1,
            ease: "none",
            yoyo: true
        }).fromTo([
            nodec, nextc, gatsbyc, graphc
        ], 2, {
            y: -10,
            repeat: -1,
            ease: "none",
            yoyo: true
        }, {
            y: 0,
            repeat: -1,
            ease: "none",
            yoyo: true
        }, "-=0");

    }, [])
    return (
        <section className="tech-services">
            <div className="tech-head">
                <h2>Our Technologies</h2>
            </div>
            <div className="technologies">
                <div className="tech-been"></div>
                <div className="tech-dots"></div>
                <div className="technologies-primary">
                    <div>
                        <div className="tech-container">
                            <div className="circle-tech react-color " ref={el => {
                                reactc = el
                            }}>
                                <img src={require("../../images/reactjs.png")} alt="GeeksHub-Media" loading="lazy"/>
                            </div>
                            <div className="circle-tech nodejs-color" ref={el => {
                                nodec = el
                            }}>
                                <img src={require("../../images/nodejs.png")} alt="GeeksHub-Media" loading="lazy"/>
                            </div>
                            <div className="circle-tech mysql-color" ref={el => {
                                sqlc = el
                            }}>
                                <img src={require("../../images/MySQL.png")} alt="GeeksHub-Media" loading="lazy"/>
                            </div>
                            <div className="circle-tech next-color" ref={el => {
                                nextc = el
                            }}>
                                <img
                                    src={require("../../images/next-js-logo-7929BCD36F-seeklogo.com.png")}
                                    alt="GeeksHub-Media" loading="lazy"
                                />
                            </div>
                            <div className="circle-tech mongo-color" ref={el => {
                                mongoc = el
                            }}>
                                <img
                                    src={require("../../images/mongo-db-design-mongodb-logo-mongodb-11562879783zbmzcchmau.png")}
                                    alt="GeeksHub-Media" loading="lazy"
                                />
                            </div>
                            <div className="circle-tech gatsby-color " ref={el => {
                                gatsbyc = el
                            }}>
                                <img src={require("../../images/gatsby.png")} alt="GeeksHub-Media" loading="lazy"/>
                            </div>
                            <div className="circle-tech rest-color" ref={el => {
                                restc = el
                            }}>
                                <img src={require("../../images/rest-api.png")} alt="GeeksHub-Media" loading="lazy"/>
                            </div>
                            <div className="circle-tech graph-color" ref={el => {
                                graphc = el
                            }}>
                                <img src={require("../../images/graphql.png")} alt="GeeksHub-Media" loading="lazy"/>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div className="tech-list-main">
                <div className="tech-list">
                    <div className="tech-primary">
                        <div>
                            <ul>
                                <li>AMP</li>
                                <li>AngularJS</li>
                                <li>flutter</li>
                                <li>Ruby on Rails</li>
                                <li>React Native</li>
                                <li>Docker</li>
                                <li>Laravel</li>
                                <li>MEAN & MERN</li>
                                <li>JAMstack</li>
                                <li>Python</li>
                                <li>Sanity CMS</li>
                                <li>Contently</li>
                                <li>.NET</li>
                                <li>Apache</li>
                                <li>AWS CloudFormation</li>
                                <li>AWS Comprehend</li>
                                <li>AWS EC2</li>
                                <li>AWS Elasticsearch</li>
                            </ul>
                        </div>
                        <div>
                            <ul>
                                <li>AWS Lambda</li>
                                <li>AWS Sagemaker</li>
                                <li>AWS S3</li>
                                <li>C#</li>
                                <li>Contently</li>
                                <li>Drupal</li>
                                <li>Laravel</li>
                                <li>Dynamics</li>
                                <li>DynamoDB</li>
                                <li>EPiServer</li>
                                <li>HTML5</li>
                                <li>Java</li>
                                <li>LESS/Sass</li>
                                <li>Linux</li>
                                <li>Magento</li>
                                <li>PHP</li>
                                <li>Postgre</li>
                                <li>Salesforce Integration</li>
                            </ul>
                        </div>
                        <div>
                            <ul>
                                <li>Shopify</li>
                                <li>Sitecore</li>
                                <li>SQL/NoSQL</li>
                                <li>Vue.js</li>
                                <li>WooCommerce</li>
                                <li>Xamarin</li>
                                <li>Typescript</li>
                                <li>Apollo Graphql</li>
                                <li>Firebase</li>
                                <li>Express</li>
                                <li>Kotlin</li>
                                <li>Elixir</li>
                                <li>ES6 Features</li>
                                <li>JSX Components</li>
                                <li>Tailwind CSS</li>
                                <li>Bulma CSS</li>
                                <li>Bootstrap 4</li>
                                <li>Materialized CSS</li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
            <div className="tech-list-main">
                <div className="tech-head-list"><h2>Consulting Services </h2></div>
                <div className="tech-list">
                    <div className="tech-primary">
                        <div>
                            <ul>
                                <li>Systems Architecture</li>
                                <li>Microservices Architecture</li>
                                <li>User Interface Design</li>
                                <li>Systems Design & Integration</li>
                                <li>User Experience Design</li>
                                <li>Enterprise Development</li>
                                <li>E-Commerce Optimization</li>
                                <li>Application Development</li>
                                <li>Architecture & Engineering</li>
                                <li>Application Maintenance</li>
                                <li>Application Security</li>
                                <li>Content Management System</li>
                                <li>CRM</li>
                                <li>Analytics</li>
                                <li>Modern Application Stacks</li>
                                <li>Digital Transformation</li>
                                <li>Product Design Sprint</li>
                                <li>Enterprise Software Development</li>
                                <li>Cloud-based Application Development</li>
                                <li>Enterprise Mobile App Development</li>
                                <li>Custom Mobile Applications</li>
                                <li>ReactNative Mobile Development</li>
                            </ul>
                        </div>
                        <div>
                            <ul>
                                <li>Documentation</li>
                                <li>Information Architecture</li>
                                <li>IT Management</li>
                                <li>Platform Migration</li>
                                <li>Process Management</li>
                                <li>Process Optimization</li>
                                <li>SaaS</li>
                                <li>System Integration</li>
                                <li>System Specification</li>
                                <li>Technology Procurement</li>
                                <li>Advanced Analytics & Reporting</li>
                                <li>Amazon Web Services (AWS)</li>
                                <li>Project Development</li>
                                <li>Testing & Quality Assurance</li>
                                <li>API & Algorithm Development</li>
                                <li>Business Intelligence</li>
                                <li>Flutter Mobile Development</li>
                                <li>Enterprise Mobile App Development</li>
                                <li>Scoping Session</li>
                                <li>Digital Transformation</li>
                                <li>Fintech</li>
                                <li>Progressive Web Apps</li>
                            </ul>
                        </div>
                        <div>
                            <ul>
                                <li>Database Solutions</li>
                                <li>Software Development & Support</li>
                                <li>Cloud Migration</li>
                                <li>Offshore/Onshore IT Management</li>
                                <li>High-Performance Computing</li>
                                <li>Systems Audit & Analysis</li>
                                <li>AI Search Solutions</li>
                                <li>Infrastructure-as-a-Service (IaaS)</li>
                                <li>Network Infrastructure</li>
                                <li>On-Demand Services</li>
                                <li>DAMS/DRM</li>
                                <li>White Label Services</li>
                                <li>Machine Learning (ML) Solutions</li>
                                <li>Marketing Automation</li>
                                <li>Transformation</li>
                                <li>Technology Roadmapping</li>
                                <li>Business Intelligence</li>
                                <li>Cloud Technology</li>
                                <li>Machine Learning</li>
                                <li>Data Annotation</li>
                                <li>Android App Development</li>
                                <li>iOS App Development</li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>

        </section>
    )
}
